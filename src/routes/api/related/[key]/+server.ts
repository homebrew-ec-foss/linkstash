...
async function buildRowWithSource(
	id: string
): Promise<{ row: LinkRow; sourceText: string; sourceHash: string } | null> {
...
async function ensureRecentEmbeddings(limit = 40) {
	const missing = await getClient().execute({
		sql: `SELECT l.id
          FROM links l
          LEFT JOIN link_embeddings le ON le.link_id = l.id
...
/**
 * GET /api/related/[key]
 * Semantic recommendations for the given link, plus group counts.
 */
export const GET: RequestHandler = async (event) => {
	...
	try {
		...
		for (const item of related) {
			const room =
				item.roomComment && String(item.roomComment).trim()
